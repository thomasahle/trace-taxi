...
describe("textFromContent", () => {
  ...
  it("should extract text from array of objects with text.value field", () => {
    expect(textFromContent([{ type: "text", text: { value: "Nested" } }])).toBe(
      "Nested",
    );
  });

  it("should extract text from array of objects with value field", () => {
    expect(textFromContent([{ value: "Direct value" }])).toBe("Direct value");
  });

  it("should extract text from array of objects with content field", () => {
    expect(textFromContent([{ content: "Content field" }])).toBe(
      "Content field",
    );
  });
  ...
  it("should handle object with text.value field", () => {
    expect(textFromContent({ text: { value: "Nested object" } })).toBe(
      "Nested object",
    );
  });
  ...
  it("should trim whitespace from array results", () => {
    expect(textFromContent(["  ", { text: "Valid" }, "  "])).toBe("Valid");
  });
...
describe("groupToolPairs", () => {
  ...
  it("should handle tool_call_id that includes the use id", () => {
    const events = [
      ...
      {
        kind: "tool-result",
        tool_call_id: "prefix-abc-suffix",
        output: "result",
      },
    ];
...
describe("cn", () => {
  ...
  it("should handle conditional classes", () => {
    ...
    expect(cn("base", condition1 && "conditional", "always")).toBe(
      "base always",
    );
    expect(cn("base", condition2 && "conditional")).toBe("base conditional");
  });
