...
{"role":"user","content":"Normal message"}
    ...
    it('should handle deeply nested or circular content', () => {
      ...
      const trace = JSON.stringify({ role: 'user', content: JSON.stringify(deepContent) });
...
{"role":"user","content":"Valid"}
  ...
  describe('Real-world trace files', () => {
    it('should parse trace file with many different tools', () => {
      ...
      const fs = require('fs');
      const path = '/Users/ahle/.claude/projects/-Users-ahle-repos-trace-taxi/c538bc9a-6fbd-4d4f-8231-428b51119092.jsonl';
      ...
      const assistantMessages = result.events.filter(e => e.kind === 'assistant');
      expect(userMessages.length).toBeGreaterThan(0);
      expect(assistantMessages.length).toBeGreaterThan(0);

      // Verify file-history-snapshot entries are skipped
      const lines = traceContent.split('\n').filter(l => l.trim());
      const snapshotLines = lines.filter(l => l.includes('"type":"file-history-snapshot"'));
