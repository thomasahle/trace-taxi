...
describe('threads store', () => {
  ...
  const createMockTraceData = (title: string = 'Test Trace', eventCount: number = 5): TraceData => ({
    title,
  ...
  describe('initialization', () => {
    ...
    it('should load threads from localStorage on initialization', async () => {
      ...
      localStorageMock.setItem('trace-viewer-threads', JSON.stringify(storedThreads));
  ...
  describe('add', () => {
    ...
    it('should handle events array being undefined', () => {
      const data = { title: 'No Events', events: undefined, originalMessages: [] } as any;
      threads.add(data);
  ...
  describe('update', () => {
    ...
    it('should persist updates to localStorage', () => {
      ...
      const stored = JSON.parse(localStorageMock.getItem('trace-viewer-threads')!);
    ...
    it('should not affect other threads', () => {
      ...
      const updated = get(threads);
      expect(updated.find(t => t.id === thread1Id)!.title).toBe('Updated Thread 1');
