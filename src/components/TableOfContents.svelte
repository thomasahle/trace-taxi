...
<ScrollArea class="flex-1 p-1 pb-24 h-full">
  {#each conversationMessages as item, index}
    {@const isUser = item.event.kind === "user"}
    {@const isSelected = index === selectedIndex}
    {@const isScrollspy = index === scrollspyIndex}
    <button
      class="flex items-center gap-2 w-full px-2 py-1.5 mb-px rounded border-0 cursor-pointer text-left transition-colors {isSelected
        ? 'bg-active'
        : 'hover:bg-accent/50'}"
      on:click={() => {
        selectedIndex = index;
